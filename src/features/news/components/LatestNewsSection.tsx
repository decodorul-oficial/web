          ...
          {featured ? (
            <article className="grid grid-cols-5 gap-6">
              ...
              <div className="col-span-3">
                ...
                <p className="mt-2 line-clamp-3 text-gray-600">{featured.content?.summary ?? featured.content?.body?.slice(0, 160)}</p>
            ...
            {rest.slice(0, 4).map((n) => (
              <li key={n.id} className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="line-clamp-2 text-xs text-gray-500">{n.content?.summary ?? n.content?.body?.slice(0, 90)}</p>
                </div>
              </li>
            ))}
          </ul>
          ...
          {rest.map((n) => (
            <article key={n.id} className="grid grid-cols-5 gap-4 py-4">
              ...
              <div className="col-span-4">
                ...
                <p className="line-clamp-2 text-sm text-gray-600">{n.content?.summary ?? n.content?.body?.slice(0, 180)}</p>
              </div>
            </article>
          ))}
        </div>
